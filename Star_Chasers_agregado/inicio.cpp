...
void inicio::setup_scene2()
{
   ...
   // personaje_ = new jugador1(true);
    //personaje_->set_scale(tam,tam);
    jugadores[0]->setPos(0 ,0);
   // personaje_->set_imagen();
    scene2->addItem(jugadores[0]);

    //creacion personaje 2
    //personaje2_ = new jugador1(false);
    //personaje2_->set_scale(tam,tam);
    jugadores[1]->setPos(0, ui->View2->height()-tam);
    //personaje2_->set_imagen();
    scene2->addItem(jugadores[1]);
...
{ //Funcion para generar el movimiento del personaje a partir de las teclas
    ...
    if (tecla-> key() == Qt:: Key_W) {  //movimiento hacia arriba
        if(jugadores[0]->y()>0){
            jugadores[0]->movimientoJugador(true);}
    }

    else if (tecla-> key() == Qt:: Key_S) { //movimiento hacia abajo
        if (jugadores[0]->y()+(tam) < ui->View2->height()-2){
            jugadores[0]->movimientoJugador(false);
        }
    }

    else if (tecla->key() == Qt:: Key_A){
        if (jugadores[0]->x()>0){
            jugadores[0]->movimientoJugador_AD(true);
        }
    }

    else if ( tecla->key() == Qt:: Key_D){
        if (jugadores[0]->x() < ui->View2->width()-2){
            jugadores[0]->movimientoJugador_AD(false);
        }
    }
    ...
    //ESTOS BOTONES SON PARA GENERAR LOS PROYECTILES DE LOS PERSONAJES
        ...
        proyect_->setPos(jugadores[0]->x()+tam*0.38, jugadores[0]->y()+tam*0.38); //Se ubica el proyectil en la posición del personaje
        proyect_->set_imagen();        
